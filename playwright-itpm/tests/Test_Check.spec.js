...
class TranslatorPage {
  ...
  async navigateToSite() {
    try {
      ...
      // Verify page is ready by checking for input field
      await this.page.waitForSelector(CONFIG.selectors.inputField, { timeout: 10000 });
    } catch (e) {
      await this.page.screenshot({ path: 'error-navigation.png', fullPage: true });
      throw new Error(`Failed to navigate to ${CONFIG.url}: ${e.message}`);
    }
  }

  async getInputField() {
    return this.page.locator(CONFIG.selectors.inputField);
  }

  async getOutputField() {
    // Get the output div by finding the whitespace-pre-wrap that is NOT a textarea
    return this.page.locator(CONFIG.selectors.outputContainer).filter({ 
      hasNot: this.page.locator('textarea') 
    }).first();
  }
  ...
  async clearAndWait() {
    try {
      ...
      throw new Error(`Failed to clear input field: ${e.message}`);
    }
  }
  ...
  async typeInput(text) {
    try {
      ...
      const filledText = await input.inputValue();
      if (filledText !== text) {
        throw new Error(`Input mismatch: expected "${text}" but got "${filledText}"`);
      }
    } catch (e) {
      throw new Error(`Failed to type input: ${e.message}`);
    }
  }
  ...
  async waitForOutput() {
    try {
      ...
      // Wait for output to be visible and contain non-empty text
      await output.waitFor({ state: 'visible', timeout: CONFIG.timeouts.outputWait });
      
      // Wait for the output to have actual content using custom function
      await this.page.waitForFunction(
        ...
        { timeout: CONFIG.timeouts.outputWait }
      );
      
      await this.page.waitForTimeout(CONFIG.timeouts.translation);
    } catch (e) {
      await this.page.screenshot({ path: 'error-output-wait.png', fullPage: true });
      const output = await this.getOutputField();
      const text = await output.textContent();
      throw new Error(`Failed to wait for output. Timeout after ${CONFIG.timeouts.outputWait}ms. Current output: "${text}". Error: ${e.message}`);
    }
  }
  ...
  async getOutputText() {
    try {
      ...
      throw new Error(`Failed to get output text: ${e.message}`);
    }
  }
  ...
  async performTranslation(inputText) {
    try {
      ...
      const result = await this.getOutputText();
      return result;
    } catch (e) {
      await this.page.screenshot({ path: `error-translation-${Date.now()}.png`, fullPage: true });
      throw new Error(`Translation failed for input "${inputText}": ${e.message}`);
    }
  }
}
